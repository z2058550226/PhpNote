				...
				if ($redpacket['ad_type'] == 1) {//系统红包
					...
					if ($money > $redpacket['money']) {//红包已经抢完
						...
						if ($res1 && $res2 && $res3 && $res4 && $res5 && $res6 && $res7 && $res8 && $res9 && $res10 && $res11) {
							...
							$chenggong = 1;
							$db->query("DELETE FROM b_1_ad_valid WHERE num=user_num");//删除领取完的红包
					...
					if (!$user_num || $user_num == 1) {//默认设置 红包数量+1
						...
						if ($redpacket['user_money'] > 0 && $redpacket['num'] > $redpacket['user_num']) {
							...
							//随机派发钱
							if ($redpacket['num'] - $redpacket['user_num'] > 1) { //超过2个时
								$money = usergrab($redpacket['user_money'], $redpacket['num'] - $redpacket['user_num']);
								if ($money) {
									$money = sprintf("%.4f", $money);
								} else {
									$money = 0.01;//最少派发0.01
								}
							} elseif ($redpacket['num'] - $redpacket['user_num'] == 1) {
